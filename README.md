...
| -------------------- | ---------------------------- | ----------- | ------------------------------ | ------------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| **Signup**           | `/auth/signup`               | POST        | `PutItem`        | `PK: USER#{email}`<br>`SK: PROFILE`                                                        |  Write - Creates user with uniqueness check          |
| **Login**            | `/auth/login`                | POST        | `GetItem`                      | `PK: USER#{email}`<br>`SK: PROFILE`                                                        |  Read - O(1) user lookup by email                    |
...
| **GetFeature**       | `/features/{flag}`           | GET         | `GetItem` + `Query`            | `PK: FEATURES`<br>`SK: FEATURE#{name}`<br>`PK: ENVIRONMENT#{name}`                         |  Read + Range Query - Gets metadata + environments   |
| **ListFeatures**     | `/features`                  | GET         | `Query`                        | `PK: FEATURES`                                                                             | Range Query - Lists all features                          |
| **UpdateFeatureEnv** | `/features/{flag}/env/{env}` | PUT         | `UpdateItem`      | `PK: ENVIRONMENT#{name}`<br>`SK: ENV#{env}`                                                | Point Write - Updates environment config                  |
| **DeleteFeatureEnv** | `/features/{flag}/env/{env}` | DELETE      | `DeleteItem`     | `PK: ENVIRONMENT#{name}`<br>`SK: ENV#{env}`                                                | Point Write - Deletes environment                         |
...
| **EvaluateFeature**  | `/features/evaluate`         | POST        | `GetItem`                      | `PK: ENVIRONMENT#{name}`<br>`SK: ENV#{env}`                                                |  Read -  feature evaluation          |
| **GetAuditLogs**     | `/features/{flag}/audit`     | GET         | `Query`                        | `PK: AUDIT#{name}`<br>`SK: LOGS#{timestamp}`                                               | Range Query - Lists audit logs             |
| **AuditConsumer**    | N/A (SQS trigger)            | N/A         | `PutItem`              | `PK: AUDIT#{name}`<br>`SK: LOGS#{timestamp}`                                               |  Write - Appends audit logs from SQS                 |
